...
while j<J
    ...
    [pydoxzw, ~, xzconfs] = estimateCondProbJT(2, [1, 3, 4], jtalg_do, nVars, domainCounts);
    [pydoxz, ~, zconfs] = estimateCondProbJT(2, [1, 3], jtalg_do, nVars, domainCounts); pydoxz = repmat(pydoxz, 1, 2);    
    pw = estimateCondProbJT(4, [], jtalg, nVars, domainCounts);
    
   
    %if b>0.05
    j=j+1;
    bias(j)= mean(abs(pydoxzw(1, :) - pydoxz(1, :)));
    ...
    trueProbs(:, j) = estimateTruePs(DeDsTest.data, jtalg_do, [1 3 4]);
    ...
    to=tic;
    inExp = [1 1 1 0];
    [sets, nSets] = allSubsets(nVars,3:nVars);
    sets(:, x) = true;
    ...
    test_acc(j, 3) = estimateAccuracy(cpha{end}, zezoconfs{end}, DeDsTest.data); % gt omb
    ...
    test_ll(j, 3) = estimateLogLoss(cpha{end}, zezoconfs{end}, DeDsTest.data); % gt omb
...
xlabel('mean absolute difference $|P(Y|do(X),Z,W)-P(Y|do(X), Z)|$', 'Interpreter', 'latex', 'FontSize', 18)
...
saveas(gcf, ['scenario_1_doN_' num2str(doN)], 'pdf');
...
fig_pos = fig.PaperPosition;
fig.PaperSize = [fig_pos(3) fig_pos(4)];
saveas(gcf, ['scenario_1_doN_' num2str(doN) '_quantiles'], 'pdf');
