...
N=5000;x=1; y=2;
doNs =100;%[50 100 500];
maxDoN = doNs(end);nDoNs =length(doNs);
...
for iter=1:nIters
    ...
    to=tic;
    [adag, dsj]= tetradFges12(obsDs.data, domainCounts, 'pretreat', true, 'onlyY', true);
    ...
    for iDoN =1:nDoNs
        ...
        for iSet=1:nSets
            ...
            overlapProbs(iSet,1:2, iDoN) = log2probs([logpDegivDoHw(iSet, iDoN) logpDegivDoHn(iSet, iDoN)]); 
            [cpha{iSet}, cphabar{iSet}, cpb{iSet}, zezoconfs{iSet}, zeconfs{iSet}] = overlap_cond_prob_hat(y, x, curz, [], expDs, obsDs, domainCounts, overlapProbs(iSet,1:2, iDoN));
        ...
        % overlap best sets
        [overlap_eacc, bestz] = max(eacc);
        [overlap_ell, bestzll] = min(ell);        
        ...
        %oracle
        tic
        [imbyfci, sameasCMB, pag]= contextFCI(obsDs, expData);
        ...
        [auc(iter,5, iDoN), sens(iter, 5, iDoN),  spec(iter, 5, iDoN), overProbs(:, iter, iDoN)] = estimateMetrics(curSet{bestz}, reshape(cpb{bestz}, domainCounts(y), []),  cmbconfigs{bestz}, testData);
        [auc(iter,6, iDoN), sens(iter, 6, iDoN),  spec(iter, 6, iDoN), overProbsll(:, iter, iDoN)] = estimateMetrics(curSet{bestzll}, reshape(cpb{bestzll}, domainCounts(y), []),  cmbconfigs{bestzll}, testData);
...
for iDoN=1:nDoNs
    ...
    biasFIMB = mean(cmbProbs(:, :, iDoN) - trueProbs);biasFIMB = reshape(biasFIMB, [], 1);
    biasFGESIMB = mean(mbeProbs(:, :, iDoN) - trueProbs);biasFGESIMB = reshape(biasFGESIMB, [], 1);
    biasFGESMB = mean(mbProbs(:, :, iDoN) - trueProbs);biasFGESMB =reshape(biasFGESMB, [], 1);
    biasFCIc =mean(imboProbs(:, :,iDoN)-trueProbs);biasFCIc =reshape(biasFCIc, [], 1);
    biasOverlap =mean(overProbsll(:, :,iDoN)-trueProbs);biasOverlap =reshape(biasOverlap, [], 1);

    figure; boxplot([ biasOverlap biasFIMB biasFGESIMB biasFGESMB biasFCIc]);
    title(['$N_e$: ', num2str(2*doNs(iDoN))],'interpreter', 'latex')  
    xticklabels({'Overlap', 'FindIMB', 'IMB', 'OMB', 'FCIt-IMB'})
